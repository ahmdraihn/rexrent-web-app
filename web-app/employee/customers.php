...
// Allow both Admin and Employee to access this version in the employee folder
// (Usually Employees are the ones interacting with customers)
...
if (isset($_GET['edit'])) {
    $editId = $_GET['edit'];
    $stmt = $conn->prepare("SELECT * FROM tb_pelanggan WHERE id_pelanggan = ?");
    $stmt->bind_param("s", $editId);
    $stmt->execute();
    $editCustomer = $stmt->get_result()->fetch_assoc();
    $stmt->close();
}
